...
def monitor_clam(camera_id=2, check_interval=30, min_confidence=75):
        ...
        while True:
            ...
            if current_time - last_check >= check_interval:
                ...
                if features is not None:
                    ...
                    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    print(f"[{timestamp}] ⚪ No clam detected")
...
if __name__ == "__main__":
    print("🐚 Clam Health Monitor")
    print("=" * 50)
    
    # Start monitoring
    monitor_clam( camera_id=2, check_interval=30, min_confidence=75 ) # Below the minimum confidence level it means the clam is dead